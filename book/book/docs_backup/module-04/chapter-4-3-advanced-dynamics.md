...
*   **$M(q)$** is the **Mass Matrix** (or inertia matrix). It's a symmetric, positive-definite matrix that relates joint accelerations to joint torques.
*   **$C(q, \dot{q})$** represents the **Coriolis and Centrifugal forces**. These are velocity-dependent forces that arise in rotating systems.
*   **$G(q)$** is the **Gravity Vector**, which accounts for the torques required to counteract gravity.

### The Lagrangian Formulation
One elegant way to derive these equations of motion is through **Lagrangian Dynamics**. The Lagrangian, $\mathcal{L}$, is defined as the difference between the kinetic energy ($K$) and the potential energy ($P$) of the system.
...
### Example: Deriving Equations for a Simple Pendulum
Consider a simple pendulum of length $L$ and mass $m$. The state is the angle $q = \theta$.
*   **Kinetic Energy**: $K = \frac{1}{2} I \dot{\theta}^2 = \frac{1}{2} (mL^2) \dot{\theta}^2$
*   **Potential Energy**: $P = mgL(1 - \cos\theta)$ (taking the bottom as zero potential)
*   **Lagrangian**: $\mathcal{L} = \frac{1}{2}mL^2\dot{\theta}^2 - mgL(1 - \cos\theta)$

Applying the Euler-Lagrange equation:
$$ \frac{d}{dt}(mL^2\dot{\theta}) - (-mgL\sin\theta) = \tau $$
$$ mL^2\ddot{\theta} + mgL\sin\theta = \tau $$
This is the famous non-linear equation of motion for a pendulum. Notice the non-linear $\sin\theta$ term. A PID controller, which is linear, would struggle to control this system accurately across its full range of motion.
...
Recall the equation of motion: $M(q)\ddot{q} + C(q, \dot{q})\dot{q} + G(q) = \tau$.

If we choose our control law (our motor torque $\tau$) to be:
$$ \tau = M(q) u + C(q, \dot{q})\dot{q} + G(q) $$
Where $u$ is a new command input.

Substituting this into the dynamics equation gives:
$$ M(q)\ddot{q} + C(q, \dot{q})\dot{q} + G(q) = M(q) u + C(q, \dot{q})\dot{q} + G(q) $$
$$ M(q)\ddot{q} = M(q) u $$
$$ \ddot{q} = u $$
We have canceled all the complex dynamics! The system now behaves like a simple double integrator. We can now define our new input $u$ using a simple PD controller to drive the system to a desired state ($q_d$):
$$ u = \ddot{q}_d + K_d(\dot{q}_d - \dot{q}) + K_p(q_d - q) $$
...
1.  Plan a trajectory for its ZMP.
2.  Plan a trajectory for its feet (footsteps).
3.  Calculate the required body motion (joint angles) that will produce the desired ZMP trajectory while also moving the feet to the next step.
...
1.  **Pendulum Dynamics in Python**:
    *   Create a Python class for the simple pendulum, as derived in the example.
    *   Write a function that takes the current angle `theta` and returns the gravity term `G(theta)`.
...
2.  **PID vs. Computed Torque**:
    *   Using your pendulum simulation, implement both a simple PID controller and a Computed Torque controller to drive the pendulum to a horizontal position ($\pi/2$).
    *   Compare their performance. Can the PID controller hold the pendulum at horizontal without error? How does the Computed Torque controller perform?
3.  **ZMP and the Support Polygon**:
...
3.  **Controller Types to Implement and Compare**:
